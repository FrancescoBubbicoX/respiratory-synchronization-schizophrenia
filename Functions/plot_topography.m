...
function h = plot_topography(ch_list, values, make_contour, system, ...
    plot_channels, plot_clabels, INTERP_POINTS, titolo, cbarlabel, cbarlim, cbarvisible)
    ...
    HEAD_EXTRA      = 0;  % 1/10 of the nasion-inion distance
    ...
    plot(nose_x, nose_y, 'Color', 'k', 'LineWidth',2.5);
    hold on;
    ...
    plot(ear_x_right+head_rho+offset, ear_y_right, 'Color', 'k', 'LineWidth',2.5); hold on;
    plot(ear_x_left-head_rho-offset, ear_y_left, 'Color', 'k', 'LineWidth',2.5); hold on;

    % Plotting the electrodes
    % [ch_x, ch_y] = pol2cart((pi/180).*(locations.theta(idx)+90), locations.radius(idx));
    if plot_channels, he = scatter(ch_x, ch_y, 40,'k', 'LineWidth',1.5); end
    ...
    ylim([-L, L]);  
    colormap jet
    cbar=colorbar('FontSize',12,'FontName','Calibri');   % Feel free to modify caxis after calling the function
    caxis([cbarlim(1) cbarlim(2)])
    ...
    hold off;
    h = gcf;
    title(titolo,'FontSize',12,'FontName','Calibri');
end
